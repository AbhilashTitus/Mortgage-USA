...
export function StartHere({ onNext, onProgressComplete, onGlowChange }: StartHereProps) {
  ...
  const handleScroll = useCallback(() => {
    ...
    if (track) {
      ...
      // Calculate active step based on which dot is closest to the trigger point
      let closestIndex = 0;
      ...
      dotRefs.current.forEach((ref, i) => {
        ...
        const distance = Math.abs(dotCenter - triggerPoint);
  ...
  return (
    <div ref={containerRef} className="relative max-w-5xl mx-auto py-8 px-4">
      ...
      {/* Hero */}
      <div className="text-center mb-20 pt-4 relative z-10">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-80 bg-blue-100/40 rounded-full blur-[100px] pointer-events-none" />
        <h1
          className="text-4xl sm:text-6xl font-extrabold tracking-tight mb-6 leading-[1.1] relative"
          style={{ color: "#003087" }}
        >
          Start Here.
        </h1>
        <p className="text-lg sm:text-xl text-slate-500 max-w-xl mx-auto leading-relaxed relative">
      ...
      <div className="relative z-10" ref={timelineTrackRef}>
        {steps.map((step, i) => {
          ...
          return (
            <div
              ...
              `}
            >
              {/* 3-column grid: Icon | Timeline | Content — balanced visually */}
        ...
        {/* This is a single continuous line that fills from top based on scroll */}
        <div 
          className="absolute left-1/2 top-0 w-[3px] pointer-events-none z-10"
          style={{
      ...
      <div className="fixed bottom-8 right-8 z-40 hidden md:flex items-center gap-3 bg-white/90 backdrop-blur-lg shadow-xl border border-slate-200/60 rounded-full px-4 py-2.5">
        <div className="w-24 h-1.5 bg-slate-200/60 rounded-full overflow-hidden">
          <div 
      ...
      <div ref={ctaContainerRef} className="relative z-10 h-[250vh] mt-[30vh]">
        <div className="sticky top-1/2 -translate-y-1/2 pt-16 pb-8">
          <div
            className="rounded-[2.5rem] p-1 shadow-2xl overflow-hidden transform hover:scale-[1.02] transition-transform duration-500 max-w-4xl mx-auto"
            style={{
              background: "linear-gradient(135deg, #003087 0%, #0072C6 50%, #77BC1F 100%)",
            }}
          >
            <div className="bg-white/95 backdrop-blur-xl rounded-[2.35rem] px-8 py-12 sm:px-16 sm:py-16 text-center relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center">
                <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mb-6 shadow-inner">
                  <FileText className="w-10 h-10" style={{ color: "#003087" }} />
                </div>
                
                <h3 className="text-3xl sm:text-4xl font-extrabold mb-4 tracking-tight" style={{ color: "#003087" }}>
                  Ready to find your number?
                </h3>
                
                <p className="text-slate-500 text-lg max-w-md mx-auto leading-relaxed mb-8">
                  Click <strong className="text-slate-700">Your Info</strong> in the sidebar to enter your financial details and unlock your personalized dashboard.
                </p>
                ...
                <button 
                  ...
                  className="inline-flex items-center justify-center gap-3 bg-slate-900 text-white px-8 py-4 rounded-full font-bold shadow-lg shadow-slate-900/20 hover:bg-slate-800 transition-all duration-300 group cursor-pointer"
                >
